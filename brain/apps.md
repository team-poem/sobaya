...
| gyowon-eval-aggregation | faculty evaluation score aggregation script | Python | paused | local only |
| kordoc-hwp-markdown | HWP → markdown conversion pipeline (report bot) | Docker | active | team-poem/report-bot |
| mydex-app-pg-poc | MYDEX JSP app PostgreSQL port PoC | JSP | imported | local only |
