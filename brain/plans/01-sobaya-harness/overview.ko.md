...
- `README.md`(한국어) + `docs/guide.md`(한국어)
...
├── CLAUDE.md                  # 하네스 계약 (EN)
├── README.md                  # 가이드 (KO)
├── .gitignore                 # apps/* (단 !apps/.gitkeep), OS 잡파일
├── .claude/
│   ├── settings.json          # 훅 연결
│   ├── hooks/
...
**`codebase/noodle-reference.md`** — 출처 노트: 소스 저장소 URL, 분석한 커밋, 빌린 것(볼트 구조, reflect/meditate, 훅, 컨벤션 표), 의도적으로 버린 것, 더 이식할 때 어디를 봐야 하는지.
