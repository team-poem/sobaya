...
*Kent Beck의 TDD 위에 세운 에이전틱 엔지니어링 워크스페이스: 사람이 spec과 실패 테스트를 전부 쓰고,*<br>
...
앱 `CLAUDE.md`에 하네스가 실행할 명령줄을 선언합니다. Kent Beck이 `agent.md`에 `cargo fmt`·`cargo test`를 둔 자리입니다:
...
- **Kent Beck** — `tdd-set/CLAUDE.md`는 그의 BPlusTree3 `rust/docs/CLAUDE.md` 원문(커밋 `e1f539e`); 플랜 체크리스트 아이디어는 그의 TCRSkill `plan.md`에서; 앱 `CLAUDE.md`의 명령줄은 그의 `agent.md`를 따름
