...
record)
  ...
  printf '%s' "$out" | jq -c --arg run "$run" --argjson iter "$iter" --arg ts "$(date -u +%FT%TZ)" '{
    ...
    denied: ((.permission_denials // []) | map(.tool_input.command // .tool_name // "?") | .[0:3])
  }' >> "$log"
  ;;
summary)
  ...
  jq -r --arg run "$run" 'select(.run == $run) |
    "\(.iter)\t\(.model)\t\(.in)\t\(.out)\t\(.cache_read)\t\(.cache_create)\t\(.cost * 1000 | round / 1000)\t\(.turns)\t\(.subagents)\t\(.denials // 0)\t\(.ms / 1000 | round)s"' "$log" \
    | { printf 'iter\tmodel\tin\tout\tcache_read\tcache_create\tusd\tturns\tsubagents\tdenials\ttime\n'; cat; } | column -t -s $'\t'
