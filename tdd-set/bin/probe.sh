...
# usage: bin/probe.sh <package-dir> <snippet-file>      (snippet = the test func only)
#        bin/probe.sh <package-dir> - <<'EOF' ... EOF    (snippet on stdin)
...
out=$(go test "./$dir" -run "^$name\$" 2>&1); rc=$?
if [ $rc -eq 0 ]; then
