---
description: Phase 0 for one app — draft apps/<name>/plan.md (split, cases, each probed red), then ask once
---
App: `apps/$ARGUMENTS`. Run the `tdd` skill's Phase 0 for the feature in `apps/$ARGUMENTS/spec.md`.
Draft `apps/$ARGUMENTS/plan.md`: split into small features, enumerate as many cases as you can,
