...
| `bin/next.sh` | Steps 6 and 7, O(1). `next.sh apps/<name>` prints only the next unchecked failed-test.md entry (heading, header block, checkbox line, code block); `next.sh apps/<name> check <TestName>` flips that one line. `/go` never reads failed-test.md whole — with N entries that was O(N²) tokens per run |
| `bin/probe.sh` | Steps 3 to 4. Drops one candidate test into the package as a temporary file, runs it, deletes it. RED (fails or does not build) → record it in failed-test.md; GREEN → the behavior already exists, drop it. Go (`go test -run`, snippet = the function preceded by its own `import (...)` block; `testing` is added only when the snippet has none) and Node (vitest or `node --test`, snippet = one `test(...)` block, header block via the third argument) |
| `bin/loop.sh` | Steps 6, 7, 10, 11. Only ever runs "go", with `SOBAYA_LOOP=1` exported so `inject-brain.sh` skips the vault index. Halts when an iteration adds entries (nobody inside the loop can be asked), stops after three iterations without a commit, prints the run's token/cost summary, then runs the gate. Cycles run on sonnet by default (25/25 green for $13.76 on 2026-09-04, see `brain/codebase/loop-cost-measurement.md`); `SOBAYA_MODEL=<model>` overrides, empty = CLI default |
...
| `bin/gate.sh` | Steps 6 and 12. PASS = failed-test.md 100% checked · suite green · no existing line touched in a test file or anywhere under `tests/`, `test/`, `__tests__/` (helpers and fixtures are frozen with the tests) · every checked entry's code block found verbatim in the suite. The human's tests are the spec, so nothing else is judged |
...
SOBAYA_MODEL= tdd-set/bin/loop.sh apps/<name> 30         # cycles default to sonnet; empty = the CLI's default model
