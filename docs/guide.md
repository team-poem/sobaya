...
1. "todo 앱 하나 만들자" → `tdd-set/bin/install.sh apps/todo-app`: 디렉터리와 git init(없을 때만), 앱 AGENTS.md(명령줄 4개 + TDD 규칙), spec/plan 템플릿, 스킬·명령·훅 설치. 기존 앱에도 같은 한 줄이고 멱등입니다. 끝나면 `brain/apps.md`에 한 줄 등록.
