...
## 시나리오 1 — 앱에 tdd-set 붙이고 첫 기능

1. "todo 앱 하나 만들자" → `tdd-set/bin/install.sh apps/todo-app`: 디렉터리와 git init(없을 때만), 앱 AGENTS.md(명령줄 4개 + `Skills:` 줄), spec/plan 템플릿. 이 셋이 앱이 갖는 전부이고, 스킬·명령·훅·TDD 규칙은 루트 것을 씁니다. 기존 앱에도 같은 한 줄이고 멱등입니다. 끝나면 `brain/apps.md`에 한 줄 등록.
2. 사용자가 앱 루트 `spec.md`(Goal·Must·Must not)를 채웁니다. 에이전트가 대신 쓰지 않습니다. 이게 끝나기 전엔 구현 시작 금지.
3. `/sobaya-plan todo-app` — 루트 세션에서. 에이전트가 초안을 만듭니다: 큰 기능을 작은 기능으로 쪼개고, 케이스를 최대한 열거하고, 하나씩 `tdd-set/bin/probe.sh`로 돌려 RED인 것만 앱 루트 `failed-test.md`에 **완전한 실패 테스트 코드**로 적습니다(항목 = 체크박스 한 줄 + 코드 블록). GREEN이면 이미 있는 행위라 적지 않습니다. 그리고 **한 번만 묻습니다**: "검토·추가 끝나셨으면 이대로 진행할까요?" 사람은 `failed-test.md`를 직접 고치고 케이스를 더합니다. 검토했는지 검증하는 장치는 없습니다. 사람을 믿습니다. 이 아키텍처는 사람이 테스트 케이스를 얼마나 잘 두느냐로 돌아갑니다.
4. 구현: 루트에서 `/go todo-app`을 사이클마다, 또는 `tdd-set/bin/loop.sh apps/todo-app` — "go" 한 번에 항목 하나: 문서의 테스트 코드를 그대로 스위트에 옮기고 Red → Green → 체크 → 커밋(행위 변경), 그다음 green 상태에서 앱 `Skills:` 줄이 고른 스킬로 Refactor → 별도 커밋(구조 변경). 커밋 없는 반복 3회면 정체로 멈춥니다. 끝나면 `gate.sh`가 검사합니다: 전부 체크·스위트 green·기존 테스트 무변조·체크 항목의 테스트가 스위트에 존재. 사람이 쓴 테스트가 곧 spec이라 그 이상은 판정하지 않습니다. 세션 안에서 서브에이전트로 돌릴 땐 **sobaya** 디스패치 규칙대로.
